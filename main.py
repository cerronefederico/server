...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        print(f"Disconnessione o errore: {e}")
    finally:
        await websocket.close()

def taskStato(db, lock):
    while True:
        with lock:
            print("ls")
            stati=db.readStati(PLC.ipPLC.PLC1)
            print(stati)
            if not plc1.controlloStato(stati):
                plc1.aggiornaStato(stati)
                print("1s")
            stati = db.readStati(PLC.ipPLC.PLC2)
            if not plc2.controlloStato(stati):
                plc2.aggiornaStato(stati)
                print("2s")
            stati = db.readStati(PLC.ipPLC.PLC3)
            if not plc3.controlloStato(stati):
                plc3.aggiornaStato(stati)
                print("3s")
        time.sleep(1)


def taskContatori(db, lock):
    while True:
        with lock:
            print("lc")
            contatori=db.readContatori(PLC.ipPLC.PLC1)
            if not plc1.controlloContatori(contatori):
                plc1.aggiornaContatori(contatori)
                print("1c")
            contatori = db.readContatori(PLC.ipPLC.PLC2)
            if not plc2.controlloContatori(contatori):
                plc2.aggiornaContatori(contatori)
                print("2c")
            contatori = db.readContatori(PLC.ipPLC.PLC3)
            if not plc3.controlloContatori(contatori):
                plc3.aggiornaContatori(contatori)
                print("3c")
        time.sleep(2)
...
if __name__ == "__main__":
    ...
    plc3=PLC()
    lock=threading.Lock()
    thread1 = threading.Thread(target=taskStato, args=(db, lock))
    thread2 = threading.Thread(target=taskContatori, args=(db, lock))
    #thread1.start()
    #thread2.start()
    uvicorn.run(app, host="0.0.0.0", port=8000)
    thread1.join()
